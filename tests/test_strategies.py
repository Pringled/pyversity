...
def test_mmr() -> None:
    ...
    res = mmr(emb, scores, k=3, diversity=1.0, metric=Metric.COSINE, normalize=True)
    ...
    res = mmr(emb, scores, k=2, diversity=0.0, metric=Metric.COSINE, normalize=True)
...
def test_cover() -> None:
    """Test COVER strategy with various diversity and gamma settings."""
    emb = np.eye(3, dtype=np.float32)
    scores = np.array([0.1, 0.8, 0.3], dtype=np.float32)

    # Pure relevance (diversity=1): picks top-k by scores
    res = cover(emb, scores, k=2, diversity=1.0)
...
def test_dpp() -> None:
    """Test DPP strategy with various diversity settings."""
    emb = np.array([[1.0, 0.0], [0.0, 1.0], [1.0, 1.0]], dtype=np.float32)
    scores = np.array([0.1, 0.2, 0.3], dtype=np.float32)

    # diversity=0: ignore relevance, diversity-only kernel
    res = dpp(emb, scores, k=3, diversity=0.0)
